...
class I18nAPI:
    ...
    def _create_router(self) -> APIRouter:
        ...
        router.add_api_route("/languages", self.get_supported_languages, methods=["GET"])
        ...
        router.add_api_route("/user/settings", self.update_user_settings, methods=["POST"])
    ...
    async def set_language(
        ...
        if user_id:
            ...
            self._user_contexts[user_id]["timezone"] = self.i18n_service.get_current_timezone()
        ...
        for lang_tz in LANGUAGE_TIMEZONE_MAPPING.values():
            if lang_tz not in common_timezones:
                timezone_list.append({
                    ...
                    "is_current": lang_tz == self.i18n_service.get_current_timezone()
                })
    ...
    async def detect_language(self, request: LanguageDetectionRequest) -> SuccessResponse:
    ...
    async def format_datetime(self, request: DateTimeFormatRequest) -> SuccessResponse:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Failed to format datetime: {str(e)}"
            )
    ...
    async def format_number(self, request: NumberFormatRequest) -> SuccessResponse:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to format number: {str(e)}"
            )
    ...
    async def get_agent_context(
        ...
        return SuccessResponse(
            message="Agent i18n context retrieved successfully",
            data=context.dict()
        )
